...
`(Killed + Errors) / (Killed + Survived + Errors) × 100%` — errored
mutants count as detected, no-coverage mutants are excluded. A `✔` row
means at least one mutant was killed; an `x` row means all mutants
survived.
