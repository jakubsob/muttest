...
- **Pure I/O and glue code** — functions that read a file and return its contents, or just call another function. There is little logic to mutate.
- **UI rendering** — hard to assert on rendered output in an automated way; mutation testing does not replace manual review here.
- **Very slow test suites** — mutation testing multiplies your test runtime by the number of mutants. Start with fast unit tests before applying it to integration tests.
