    ...
    steps:
      ...
      - name: Run mutation tests
        ...
        run: |
          ...
          muttest::muttest("tests/testthat", plan)
```
...
To enforce a minimum mutation score, parse the output or use R's exit codes. A simple approach with `tryCatch`:

```r
results <- muttest::muttest("tests/testthat", plan)

score <- attr(results, "score")  # check actual attribute name in ?test
if (score < 0.7) {
...
- **Cache R package dependencies** — `r-lib/actions/setup-r-dependencies@v2` caches dependencies automatically; make sure the cache key includes your `DESCRIPTION` file.
