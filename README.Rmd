...
[covr](https://github.com/r-lib/covr) tells you which lines were executed. It cannot tell you whether your assertions are strong enough to catch a real bug. A test suite full of `expect_true(is.numeric(x))` checks will reach 100% coverage while missing every meaningful failure.
...
The mutation score is: $\text{Mutation Score} = \frac{\text{Killed} + \text{Errors}}{\text{Killed} + \text{Survived} + \text{Errors}} \times 100\%$, where a Mutant is a variant of the original code used to test the robustness of the test suite. An erroring mutant was still detected, so it counts as killed; mutants with no test coverage are excluded.
